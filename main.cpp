...
Mat transf(Mat src)
{
   ...
   Point2f dstTri[3];
    ...
    float scale_r;
    float scale_r_n;
    float scale_gain=0.15f;///Change here
    float trans_x_r;
    float trans_y_r;
...
///
  ...
  //  rand_img_indx = rand() % (custom_dataset_size / nr_of_classes);
    ...
    std::string output_file = std::to_string(rand_img_indx);
  ...
  for(int i=0;i<nr_of_files;i++)
  {
    ...
    for(int r=0;r<nr_randomize_images;r++)
    {
      ...
      cv::Mat imgSquarRect = makeSquareImg(img, random_sized_img, input_file);
      printf("output_file = ");
